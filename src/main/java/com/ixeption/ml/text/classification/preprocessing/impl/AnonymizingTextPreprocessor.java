...
public class AnonymizingTextPreprocessor implements TextPreprocessor {
    ...
    public enum ReplaceRegEx {
        ...
        PATTERN_IBAN(           "PLACEHOLDER_IBAN",       Pattern.compile("[a-s-A-Z]{2} ?\\d{2}[ \\t]?([0-9a-zA-Z]{2,4}\\s?){3,6}[ \t]")),
        PATTERN_LONG_NUMBER(    "PLACEHOLDER_LONG_NUMBER",        Pattern.compile("(\\+)*([0-9]{3,}).*([0-9]{3,})", Pattern.CASE_INSENSITIVE)),
        ;
        // @formatter:on
